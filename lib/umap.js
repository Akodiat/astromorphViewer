...
//UMAP.reshape2d = UMAP.rejectionSample = UMAP.max2d = UMAP.max = UMAP.mean = UMAP.sum = UMAP.linear = UMAP.ones = UMAP.zeros = UMAP.filled = UMAP.range = UMAP.empty = UMAP.norm = UMAP.tauRand = UMAP.tauRandInt = void 0;
...
function makeHeap(nPoints, size) {
    var makeArrays = function (fillValue) {
        return empty(nPoints).map(function () {
...
class SparseMatrix {
    ...
    toArray () {
        var _this = this;
        var rows = empty(this.nRows);
        var output = rows.map(function () {
            return zeros(_this.nCols);
        });
        this.entries.forEach(function (value) {
    ...
    transpose() {
      ...
      var dims = [this.nCols, this.nRows];
...
function makeForest(data, nNeighbors, nTrees, random) {
    ...
    var trees = range(nTrees).map(function (_, i) { return makeTree(data, leafSize, i, random); });
...
function euclideanRandomProjectionSplit(data, indices, random) {
    ...
    var hyperplaneVector = zeros(dim);
    ...
    var indicesLeft = zeros(nLeft);
    var indicesRight = zeros(nRight);
    nLeft = 0;
    nRight = 0;
...
function flattenTree(tree, leafSize) {
    ...
    var hyperplanes = range(nNodes).map(function () {
        return zeros(tree.hyperplane ? tree.hyperplane.length : 0);
    });
    var offsets = zeros(nNodes);
    var children = range(nNodes).map(function () { return [-1, -1]; });
    var indices = range(nLeaves).map(function () {
        return range(leafSize).map(function () { return -1; });
    });
...
class UMAP {
    ...
    makeSearchGraph(X) {
        ...
        var transpose = searchGraph.transpose();
    ...
    fuzzySimplicialSet(X, nNeighbors, setOpMixRatio) {
        ...
        var a = subtract(add(sparseMatrix, transpose), prodMatrix);
        var b = multiplyScalar(a, setOpMixRatio);
        var c = multiplyScalar(prodMatrix, 1.0 - setOpMixRatio);
        var result = add(b, c);
        return result;
    };
    categoricalSimplicialSetIntersection(simplicialSet, target, farDist, unknownDist) {
    ...
    smoothKNNDistance(distances, k, localConnectivity, nIter, bandwidth) {
        ...
        var rho = zeros(distances.length);
        var result = zeros(distances.length);
        for (var i = 0; i < distances.length; i++) {
            ...
            result[i] = mid;
            if (rho[i] > 0.0) {
                var meanIthDistances = mean(ithDistances);
                if (result[i] < MIN_K_DIST_SCALE * meanIthDistances) {
                    result[i] = MIN_K_DIST_SCALE * meanIthDistances;
                }
            }
            else {
                var meanDistances = mean(distances.map(mean));
    ...
    computeMembershipStrengths(knnIndices, knnDistances, sigmas, rhos) {
        ...
        var rows = zeros(nSamples * nNeighbors);
        var cols = zeros(nSamples * nNeighbors);
        var vals = zeros(nSamples * nNeighbors);
        for (var i = 0; i < nSamples; i++) {
            for (var j = 0; j < nNeighbors; j++) {
                var val = 0;
                if (knnIndices[i][j] === -1) {
    ...
    initializeSimplicialSetEmbedding() {
        ...
        this.embedding = zeros(graph.nRows).map(function () {
            return zeros(nComponents).map(function () {
    ...
    makeEpochsPerSample(weights, nEpochs) {
        ...
        var maxVal = max(weights);
        var nSamples = weights.map(function (w) { return (w / maxVal) * nEpochs; });
    ...
    optimizeLayoutAsync(epochCallback) {
        var _this = this;
...
function findABParams(spread, minDist) {
    ...
    var xv = linear(0, spread * 3, 300).map(
        function (val) { return (val < minDist ? 1.0 : val);
    });
    var yv = zeros(xv.length).map(function (val, index) {
...
function resetLocalConnectivity(simplicialSet) {
    ...
    var transpose = simplicialSet.transpose();
...
function initTransform(indices, weights, embedding) {
    var result = zeros(indices.length).map(
        function (z) { return zeros(embedding[0].length);
    });
    for (var i = 0; i < indices.length; i++) {
